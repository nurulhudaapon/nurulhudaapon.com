...
import { createQuestion, createVisitor, getAllQuestions, getQuestions, updateQuestion } from 'lib/database';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    if (req.method == 'POST') {
        ...
        if (!visitorId) {
            const createdVisitor = await createVisitor(req.headers['x-forwarded-for'] as string, visitor);
    ...
    // Update
    if (req.method == 'PUT') {
        const { id, answer, deleted } = JSON.parse(req.body);

        const updatedQuestion = await updateQuestion(id, answer, deleted);
    ...
    try {
        ...
        // if param all is true, return all questions
        if (req.query.all) {
